...
if [[ $# -ne 1 ]]; then
  echo "Usage: $0 polyvise|debatefrog|debatefrog-beta" >&2
  exit 1
fi
...
case "$1" in
  ...
  debatefrog-beta)
    ...
    APP_WORKSPACE="@polyvise/debatefrog-web"
    APP_DIR="apps/debatefrog-web"
    SITE_URL="${DEBATEFROG_BETA_SITE_URL:-https://beta.debatefrog.com}"
    LLM_MAX_TOKENS="${POLYVISE_LLM_MAX_TOKENS:-2200}"
    DEFAULT_REPOSITORY="firestore"
    DEFAULT_ENABLE_MOCK_LLM="false"
    DEFAULT_EVIDENCE_PROVIDER="tavily"
    APP_CHANNEL="${POLYVISE_APP_CHANNEL:-beta}"
    OPENROUTER_SECRET="${GCP_DEBATEFROG_BETA_OPENROUTER_SECRET:-${GCP_DEBATEFROG_OPENROUTER_SECRET:-debatefrog-openrouter-api-key}}"
    TAVILY_SECRET="${GCP_DEBATEFROG_BETA_TAVILY_SECRET:-${GCP_DEBATEFROG_TAVILY_SECRET:-debatefrog-tavily-api-key}}"
    ;;
  *)
    echo "Usage: $0 polyvise|debatefrog|debatefrog-beta" >&2
    exit 1
